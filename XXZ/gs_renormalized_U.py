...
def sites(L,Nmax, g):  #Define the sites of the MPS
 ...
 A_sqd = npc.tensordot(BSite.B+BSite.Bd, BSite.B+BSite.Bd, 1)
...
for chi in chis:
     ...
     rho = np.load(os.path.join("rho", "rho"+ID+".npy"))
     B = sites(L, Nmax, g)[0]
     op = B.KR.to_ndarray()
     ...
     siti = sites(L, Nmax, new_g)
     t0 = time.time()
     H, M, Mylat = calc_MPO(new_J, Omega, Nmax, new_g, U, PBC = 1)
     ...
     for i in range(L-1):
      KR += psi.correlation_function('Cd','C', [i+1], [i+2], autoJW=True)[0][0]
      #KL += psi.correlation_function('Cd','C', [i+1], [i], autoJW=True)[0][0]
     KR += psi.correlation_function('Cd','C', [L], [1], autoJW=True)[0][0]
     K = abs(KR) 
     kai = np.angle(KR)
     print(K, kai)
     np.save(os.path.join("kinetic_operator", "re_kr"+ID), K)
     np.save(os.path.join("kinetic_operator", "im_kr"+ID), kai)
     np.save(os.path.join("photon_occupation", "photon_occupation"+ID) ,psi.expectation_value("N", 0))
     #print("N = {}".format(psi.expectation_value("N", 0)))
     np.save(os.path.join("photon_occupation", "a_squared"+ID) ,psi.expectation_value("A_sqd", 0))
     #print("Asq = {}".format(psi.expectation_value("A_sqd", 0)))
