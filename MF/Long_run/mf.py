...
def XXZ_PERIODIC(L, chi, U, gamma):
    ...
    def calc_MPO(gamma, U):
      ...
      for i in range(L-1):
        MyMod.add_multi_coupling_term(strength = -gamma, ijkl = [i,i+1], ops_ijkl = ['Cd', 'C'], op_string = [ 'JW'], plus_hc=False)
        MyMod.add_multi_coupling_term(strength = -np.conj(gamma), ijkl = [i,i+1], ops_ijkl = ['C', 'Cd'], op_string = ['JW'], plus_hc=False)
...
def Iterator_MF(g, U, L, its, chi):
 gamma = 1   
 Efer, K, kai = XXZ_PERIODIC(L, chi, U, gamma)   
 for i in range(its):
 ...
 return Etot,  GA, PHI, K, kai, nph

g = float(sys.argv[1])
L = int(sys.argv[2])
U = float(sys.argv[3])
Etot,  GA, PHI, K, kai, nph = Iterator_MF(g, U, L, 4, 800)
 
ID = "_L"+"{:.2f}".format(L)+"{:.2f}".format(g)+"{:.2f}".format(U)
np.save("GAS_"+ID+".npy", GA)
np.save("PHIS_"+ID+".npy", PHI)
np.save("KS_"+ID+".npy", K)
np.save("kais_"+ID+".npy", kai)
np.save("nphs_"+ID+".npy", nph)
np.save("ETOT"+ID+".npy", Etot)
