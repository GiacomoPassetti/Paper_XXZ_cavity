...
def XXZ_PERIODIC(L, chi, U, gamma):
    ...
    Kop.apply(psi0, options)
    KR = psi0.overlap(psi)
    return E, np.abs(KR), np.angle(KR), CORR, psi
def sites(L):  #Define the sites of the MPS
      FSite=FermionSite('N', filling=0.5)
      sites=[]
      for i in range(L):
...
def kinetic_op(L):
      ...
      MyMod = CouplingModel(reg_lat)
      for i in range(L-1):
              MyMod.add_coupling_term(strength = 1, i = i,j = i+1, op_i ='C', op_j ='Cd', op_string = 'JW', plus_hc=False)
      MyMod.add_coupling_term(strength = 1, i = 0,j = L-1, op_i ='Cd', op_j ='C', op_string = 'JW', plus_hc=False)
      mpo = MyMod.calc_H_MPO()
      return mpo
