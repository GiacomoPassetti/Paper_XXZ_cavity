...
def Iterator_MF(g, U, L, its, chi):
 ...
 for i in range(its):
     ...
     Efer, K, kai = XXZ_PERIODIC(L, chi, U, gamma)
     print(Efer, K, kai)
     Ebos, b, c, gamma = Boson(K, kai, 1, g/np.sqrt(L), 1, 30)
...
Energies = []
for U in Us:
   Energies.append(Iterator_MF(g, U, L, 5, 200))

ID = "Ground_MF_L"+"{:.2f}".format(L)+"{:.2f}".format(g)
np.save(ID+".npy", Energies)
print("done", len(Energies))
