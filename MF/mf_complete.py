...
def Boson(k, phi, omega, g, t, Nmax):
    ...
    for i in range(Nmax+1):
        ...
        prj[i,i] = 1
        Phot_proj.append(v.conj().T.dot(prj.dot(v)))

    gamma, Phi = abs(exp), np.angle(exp)
    return E, gamma, Phi, exp, n_phot, dyno, A_sqd, Phot_proj
...
def Iterator_MF(g, U, L, its, chi, omega):
 ...
 for i in range(its):
     ...
     print(Efer, K, kai)
     Ebos, GA, PHI, gamma_new, nph, dyno, A_sqd, Phot_proj = Boson(K, kai, omega, g/np.sqrt(L), 1, 30)
 ...
 return Etot,  GA, PHI, K, kai, nph, CORR, dyno, A_sqd, Phot_proj
...
chi = int(sys.argv[3])
U = float(sys.argv[4])
Etot,  GA, PHI, K, kai, nph, CORR, dyno, A_sqd, Phot_proj = Iterator_MF(g, U, L, 10, chi, omega)
...
np.save(os.path.join("photon_occupation", "mean_field_Phot_projection"+ID), Phot_proj)
