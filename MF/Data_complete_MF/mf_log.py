...
def XXZ_PERIODIC(L, chi, U, gamma):
    ...
    CORR = psi.correlation_function('dN','dN', sites1=[1], sites2=[int(L/2)])
...
def Iterator_MF(g, U, L, its, chi):
 gamma = 1   
 Efer, K, kai, CORR = XXZ_PERIODIC(L, chi, U, gamma)   
 for i in range(its):
...
CORRS = []
for U in Us:
   Etot,  GA, PHI, K, kai, nph, CORR = Iterator_MF(g, U, L, 4, 300)
   GAS.append(GA)
   PHIS.append(PHI)
   KS.append(K)
   kais.append(kai)
...
ID = "log_check_L"+"{:.2f}".format(L)+"{:.2f}".format(g)
