...
def Boson(k, phi, omega, g, t, Nmax):
    ...
    return E, gamma, Phi, exp, n_phot, dyno
...
def Iterator_MF(g, U, L, its, chi):
 ...
 for i in range(its):
     ...
     Efer, K, kai, CORR = XXZ_PERIODIC(L, chi, U, gamma)
     print(Efer, K, kai)
     Ebos, GA, PHI, gamma, nph, dyno = Boson(K, kai, 1, g/np.sqrt(L), 1, 30)
     print("After sweep", i, "Gamma is :", gamma)
 Etot = Efer + Ebos + (2 * K *gamma *np.cos(kai + np.angle(gamma)))
 return Etot,  GA, PHI, K, kai, nph, CORR, dyno
...
dynos = []
for U in Us:
   Etot,  GA, PHI, K, kai, nph, CORR, dyno = Iterator_MF(g, U, L, 4, 600)
   GAS.append(GA)
   PHIS.append(PHI)
   KS.append(K)
   kais.append(kai)
