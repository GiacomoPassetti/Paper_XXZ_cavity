...
def inset_1b(ax):
    ...
    ax.bar(bins + 0.15, np.abs(phot_distr_DMRG), log=True, fc='grey', width = 0.2, edgecolor ='black', linewidth = 0.25)
    ax.bar(bins - 0.15, np.abs(phot_distr_MF), log=True, fc = 'darkorange', width = .2, edgecolor ='black', linewidth = 0.25)
    ...
    ax.set_xticklabels(['0', '1','2', '3','4', '5'], fontsize = 6)
    ax.set_ylim(.5 * 1e-10, 2. * 1e0)
    ax.set_yticks([1e0, 1e-3, 1e-6, 1e-9])
    ax.set_yticklabels(['$10^{0}$', '$10^{-3}$', '$10^{-6}$', '$10^{-9}$'], fontsize = 6)
    ax.text(2, 0.001,r"$U\left[t_{\rm h}\right] = 3t_{\rm h}$", fontsize = 6)
