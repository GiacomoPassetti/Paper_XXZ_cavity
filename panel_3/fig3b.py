...
gs = gridspec.GridSpec(nrow, ncol,
         wspace=0.0, hspace=0.0, top=0.86, bottom=0.18, left=0.15, right=0.95) 
...
for i in range(len(Ls)):
     ...
     ax1.plot(Ussr[0:32], yArr1, color = colors[2], label= r"$\rm{DMRG}$", marker='D', markeredgecolor='black', markersize = 3, markeredgewidth=0.4, lw = 1.5)
...
ax1.plot(Ussr[0:32], renormU, color = 'cadetblue', label= r"$t_{eff}$")
...
MF = MF.reshape(32)
MF = MF - data_g0
ax1.plot(Ussr[0:32], MF,label= r"$\rm{MF}$", dashes=(2, 2), ls = "--", color = "black")
...
ax1.text(0.05, 0.00152, r"$\times 10^{-3}$", fontsize = fontsize)
