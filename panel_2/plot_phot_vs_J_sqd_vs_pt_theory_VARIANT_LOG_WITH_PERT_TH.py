...
def Perturbation_theory(g, U, omega, L):
     def nF(k):
     ...
     def summand(k, p, q):
        ...
        n3 = 1 - nF(k + q)
        n4 = 1 - nF(p - q)
        fillingFac = n1 * n2 * n3 * n4
        if(np.abs(fillingFac) < 1e-14):
           return 0.
        temp1 = np.sin(k + p) + np.sin(p - q) - np.sin(p) - np.sin(k)
        temp2 = np.cos(k + p) + np.cos(p - q) - np.cos(p) - np.cos(k)
        return  temp1**2 / temp2**2 * fillingFac
     
     def nPhot():
        kArr = np.linspace(-np.pi, np.pi, L, endpoint=False)
        sum = 0.
        for k in kArr:
           for p in kArr:
              for q in kArr:
                 sum += 4 * g ** 2 * U ** 2 / (omega ** 2 * L ** 3) * summand(k, p, q)
        return sum
...
X_left = np.linspace(0, 4, 20)
Y_left = []
for U in list(X_left):
        Y_left.append(Perturbation_theory(g, U, Omega, L))
Y_left = np.array(Y_left)/L
...
ax.plot(X_left, Y_left, label = r"$FULL PERT$", color = "blue", ls = "--")
...
plt.savefig(os.path.join("plots", "plot_perturbation_theory_log_WITH_PERT.png"))
